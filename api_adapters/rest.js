  ...
  routing(options = {}) {
    ...
    router.route(`/${ viewmodel.name_plural }`)
      .get((override && override.get_index && Array.isArray(override.get_index)) ?
        override.get_index : [preTransform, middleware.load_with_count, middleware.load_with_limit, middleware.paginate, postTransform, middleware.index,])
      .post((override && override.create_item && Array.isArray(override.create_item)) ?
        override.create_item : [preTransform, this.protocol.resources.core.controller.save_revision, middleware.create, postTransform,]);
    router.route(`/${ viewmodel.name_plural }/:id`)
      .get((override && override.get_item && Array.isArray(override.get_item)) ?
        override.get_item : [preTransform, middleware.load, postTransform, middleware.show,])
      .put((override && override.update_item && Array.isArray(override.update_item)) ?
        override.update_item : [preTransform, this.protocol.resources.core.controller.save_revision, middleware.update, postTransform,])
      .delete((override && override.delete_item && Array.isArray(override.delete_item)) ?
        override.delete_item : [preTransform, middleware.load, middleware.remove, postTransform,]);
    return router;
  }
  /**
